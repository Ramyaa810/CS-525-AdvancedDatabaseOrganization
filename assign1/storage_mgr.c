...
RC appendEmptyBlock (SM_FileHandle * fHandle) {
	...
	int i;
	// Initialize a new block(page) of 4096 \0 chars
    SM_PageHandle newBlock[PAGE_SIZE];
	// Increment through block and insert 4096 \0 chars
    for(i=0;i<PAGE_SIZE;i++) {
        newBlock[i] = '\0';
    }
	// increment page count in metadata
    fHandle->totalNumPages++;
    writeBlock(fHandle->totalNumPages , fHandle , newBlock[PAGE_SIZE]);
    return RC_OK;
